...
PIPER_MODEL_ONNX = os.path.join(VOICES_DIR, "sk_SK-lili-medium.onnx")
PIPER_MODEL_JSON = os.path.join(VOICES_DIR, "sk_SK-lili-medium.onnx.json")

PIPER_EN_MODEL_ONNX = os.path.join(VOICES_DIR, "en_US-lessac-medium.onnx")
PIPER_EN_MODEL_JSON = os.path.join(VOICES_DIR, "en_US-lessac-medium.onnx.json")

if not (os.path.exists(PIPER_MODEL_ONNX) and os.path.exists(PIPER_MODEL_JSON)):
    raise FileNotFoundError(
        f"Chýba Piper model v priečinku ./voices. "
        f"Očakávam: {PIPER_MODEL_ONNX} a {PIPER_MODEL_JSON}"
    )

PIPER_VOICE = PiperVoice.load(PIPER_MODEL_ONNX, PIPER_MODEL_JSON)


def piper_tts_raw(text: str) -> str:
    """
    Vygeneruje základný WAV z Piper hlasu (bez úprav).
    Používa oficiálne API synthesize_wav → wave.open s nastavenými parametrami.
    """
    fd, wav_path = tempfile.mkstemp(suffix=".wav")
    os.close(fd)

    # Piper očakáva wave.Wave_write objekt s nastavenými parametrami
    with wave.open(wav_path, "wb") as wav_file:
        wav_file.setnchannels(1)      # mono
        wav_file.setsampwidth(2)      # 16-bit
        wav_file.setframerate(22050)  # 22.05 kHz (Lili model)
...
def piper_tts_anime(text: str, lang: str) -> str:
    ...
    Použije Lili, zvýši pitch a rýchlosť.
    """
    base_path = piper_tts_raw(text)
    ...
    if n == 0:
        # bezpečnostná brzda – vráť originál
        return base_path
    ...
    if sw == 2:
        ...
        # neznámy formát – nepokúšaj sa upravovať
        return base_path

    if nch > 1:
        data = data.reshape(-1, nch).mean(axis=1)

    # Anime nastavenie – trošku vyšší hlas a rýchlejšie tempo
    semitones = 2.8
    speed = 1.12
